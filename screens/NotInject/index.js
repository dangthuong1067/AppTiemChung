...
import { fetchInfo14 } from "../../store/slices/info14";

const NotInjectScreen = () => {
    const [searchText, setSearchText] = useState('');
    const filteredInfos = () => infoData14?.filter(eachInfo => eachInfo.HoTen.toLowerCase().includes(searchText.toLowerCase()))
    const navigation = useNavigation()
    const dispatch = useDispatch()
    const infoData14 = useSelector(state => state.Info14.info14)

    useEffect(() => {
        dispatch(fetchInfo14())
    }, [])
    // useEffect(() => {
    //     dispatch(fetchInfo1())
    // }, [])
    const showCategoryHome = ({ item }) => {
        const categoryItem = infoData14?.find(category => category.MaDK === item.MaDK)
...
export default NotInjectScreen
